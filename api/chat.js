...
async function callClaude(messages, apiKey, systemPrompt, maxTokens) {
...
async function callGemini(messages, apiKey, systemPrompt, model, maxTokens) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Serious mode gets a stronger model and more room to write complete answers.
    // Casual/Gus mode stays on the cheap model since jokes are short.
    const isSerious = mode === "serious";
    const geminiModel = isSerious ? "gemini-flash-latest" : "gemini-flash-lite-latest";
    const maxTokens = isSerious ? 1600 : 500;
    ...
    if (provider === "gemini") {
      ...
      reply = await callGemini(messages, apiKey, systemPrompt, geminiModel, maxTokens);
    } else {
      ...
      reply = await callClaude(messages, apiKey, systemPrompt, maxTokens);
    }

    // ---- 5. Save conversation history for Serious mode only (Gus Plus memory) ----
    // Casual mode intentionally starts fresh each session (part of the fun),
    // but Serious mode remembers you across visits — a real reason to subscribe.
    if (isSerious) {
      const updatedMessages = [...messages, { role: "assistant", content: reply }].slice(-40);
      await supabaseAdmin
        .from("conversation_history")
        .upsert(
          { user_id: userId, mode: "serious", messages: updatedMessages, updated_at: new Date().toISOString() },
